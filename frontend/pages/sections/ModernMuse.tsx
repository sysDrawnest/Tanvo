    ...
    return (
        <section className="relative w-full overflow-visible z-10">
            ...
            <div className="relative w-full h-[600px] md:h-[80vh] lg:h-[90vh] min-h-[550px] max-h-[950px] overflow-hidden bg-[#0A0A0A]">
                ...
                <div className="absolute top-1/2 left-4 -translate-y-1/2 md:left-[8%] lg:left-[12%] w-[58%] sm:w-[50%] md:w-full md:max-w-[460px] lg:max-w-[540px] z-20">
                    <motion.div 
                        initial={{ opacity: 0, y: 30 }}
                        whileInView={{ opacity: 1, y: 0 }}
                        transition={{ delay: 0.2, duration: 1.2, ease: [0.16, 1, 0.3, 1] }}
                        viewport={{ once: true }}
                        className="relative w-full bg-black/20 backdrop-blur-md border border-white/10 rounded-none p-4 sm:p-10 md:p-12 lg:p-14 text-center shadow-[0_25px_60px_-15px_rgba(0,0,0,0.35)] flex flex-col items-center justify-center"
                    >
                        {/* Brand Label */}
                        <span className="font-sans text-[9px] md:text-xs tracking-[0.4em] uppercase text-white/80 mb-3 md:mb-6 block font-light">
                            TANVO PRESENTS
                        </span>

                        {/* Heading - Elegant Serif Stacked Title */}
                        <h2 className="font-serif text-2xl sm:text-3xl md:text-5xl lg:text-6xl xl:text-7xl leading-[1.08] mb-4 md:mb-6 text-white tracking-tight uppercase">
                            THE MODERN
                            <br />
                            MUSE
                        </h2>
                        ...
                        {/* Description */}
                        <p className="font-sans text-[10px] sm:text-xs md:text-sm lg:text-base text-white/95 max-w-lg mx-auto leading-relaxed font-light">
                        ...
                        <div className="absolute bottom-[-24px] left-1/2 transform -translate-x-1/2 z-30">
                            <Link 
                                ...
                                className="group relative inline-flex items-center gap-2 px-6 py-3 bg-[#5A040B] border border-white/10 text-white font-sans text-[10px] sm:text-xs uppercase tracking-[0.2em] font-semibold rounded-none shadow-[0_12px_30px_rgba(90,4,11,0.4)] transition-all duration-300 hover:bg-[#72050E] hover:shadow-2xl active:scale-95 whitespace-nowrap"
                            >
                                <span className="relative z-10 flex items-center gap-2">
                                    Explore Collection 
                                    <span className="text-[11px] sm:text-[13px] group-hover:translate-x-1 transition-transform duration-300">→</span>
                                    {/* Traditional Spool SVG Icon */}
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="w-3 h-3 sm:w-3.5 sm:h-3.5 inline-block text-[#C9A84C] group-hover:rotate-12 transition-transform duration-300">
